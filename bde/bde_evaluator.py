...
class BdePredictor:
    ...
    def _predict_regression(
        ...
        if credible_intervals:
            # this diregards the sigmas and uses only the mus which is an ok strategy (the cheapest one, could be a default)
            # if you want to incorporate the sigmas you could sample for example n (n=1 for more than 1000 samples and n=10 for less than 1000 samples) 
            # predictions from each mu,sigma (gaussian) pair and then compute the quantiles over all these sampled predictions - likely a better
